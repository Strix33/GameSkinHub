  ...
  const fetchPendingRequests = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('sell_requests').select('*').eq('status', 'pending').order('created_at', {
        ascending: false
      });
      if (error) {
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {requests.map(request => <div key={request.id} className="border rounded-lg p-4 space-y-3">
                ...
                <div className="text-sm bg-muted p-3 rounded space-y-2">
                  ...
                  <div className="pt-2 border-t">
                    ...
                    {request.verification_method === 'discord' && <p><strong>Discord Username:</strong> {request.discord_username}</p>}
                  </div>
                </div>
                
                {/* Discord username input for checker (only if verification method is discord) */}
                {request.verification_method === 'discord' && <div className="mb-3">
                    <label className="block text-sm font-medium mb-1">
                      Checker Discord Username *
                    </label>
                    <input type="text" placeholder="your_discord#1234" value={checkerDiscordUsername[request.id] || ''} onChange={e => setCheckerDiscordUsername({
                ...checkerDiscordUsername,
                [request.id]: e.target.value
              })} className="w-full px-3 py-2 border rounded-md text-sm bg-gray-800" />
            ...
            {requests.length === 0 && <p className="text-muted-foreground text-center py-8">No pending requests to review</p>}
          </div>
        </CardContent>
      </Card>
    </div>;
